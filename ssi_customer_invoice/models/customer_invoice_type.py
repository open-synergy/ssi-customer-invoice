...
class CustomerInvoiceType(models.Model):
    """
    Represents a configuration template for a category of customer invoices.

    Stores the default journal and receivable account used when posting
    a customer invoice of this type, plus three independent allow-lists
    (product, currency, pricelist) that a customer invoice document of
    this type may use. Each allow-list supports three selection
    strategies -- manual (explicit recordset), domain (ORM domain
    string), or code (Python snippet) -- but the strategy is only
    *stored* here; evaluating it is the responsibility of the document
    side via ``mixin.many2one_configurator``. This model has no methods
    of its own.
    """

    _name = "customer_invoice_type"
    _inherit = ["mixin.master_data"]
    _description = "Customer Invoice Type"
    ...
    journal_id = fields.Many2one(
        ...
        required=True,
        ondelete="restrict",
        help="Default journal used when posting a customer invoice of " "this type.",
    )
    receivable_account_id = fields.Many2one(
        ...
        required=True,
        ondelete="restrict",
        help="Default receivable account used when posting a customer "
    ...
    product_selection_method = fields.Selection(
        ...
        default="domain",
        required=True,
        help="Method used to determine which products can be selected on "
        "a customer invoice of this type.",
    )
    product_ids = fields.Many2many(
        string="Products",
        comodel_name="product.product",
        relation="rel_customer_invoice_type_2_product",
        column1="type_id",
        column2="product_id",
    ...
    currency_selection_method = fields.Selection(
        ...
        default="domain",
        required=True,
        help="Method used to determine which currencies can be selected "
        "on a customer invoice of this type.",
    )
    currency_ids = fields.Many2many(
        string="Currencies",
        comodel_name="res.currency",
        relation="rel_customer_invoice_type_2_currency",
        column1="type_id",
        column2="currency_id",
    ...
    pricelist_selection_method = fields.Selection(
        ...
        default="domain",
        required=True,
        help="Method used to determine which pricelists can be selected "
        "on a customer invoice of this type.",
    )
    pricelist_ids = fields.Many2many(
        string="Pricelists",
        comodel_name="product.pricelist",
        relation="rel_customer_invoice_type_2_pricelist",
        column1="type_id",
